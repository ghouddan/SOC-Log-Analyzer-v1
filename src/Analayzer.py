...
from detectors import detect_brute_force, detect_blacklist, detect_of_hours_login # Ensure detectors.py exists in the same directory
...
parser.add_argument("--timestamp", help="Filter log entries based on timestamp.", action="store_true")
...
if args.file_type == "auth":
    ...
    if args.ip_list:
        detect_blacklist(args.ip_list,auth_log, args.output)
    if args.timestamp:
        detect_of_hours_login(auth_log,args.output)
